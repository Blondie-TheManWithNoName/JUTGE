...
void escriure_pila_parells (pila b, bool &g)
{
  while (not b.es_buida())
  {
    if (b.cim()%2 == 0)
    {
      if (g)
...
void escriure_pila_senars(pila b, bool &g)
{
	while (not b.es_buida())
	{
		if (b.cim()%2 != 0)
  ...
  bool g = 0;
	...
	while (getline(cin, str))
	{
		...
		a.apilar(x);
			...
			escriure_pila_parells(a, g);
			escriure_pila_senars(a, g);
			while (not a.es_buida()) a.desapilar();
      g = 0;
			cout << endl;
	}
}
